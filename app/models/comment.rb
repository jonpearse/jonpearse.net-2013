  ...
  private
    ...
    def process_blockquote
      ...
      self.comment_rendered = comment_rendered.gsub(/<p>&gt;(.*?)<\/p>/m) do |match|
        ...
        "<blockquote class=\"u-box -smoke o-comment__quotation\" cite=\"http://jonpearse.net#{cite_url}\" data-comment-id=\"#{self.reply_to_id}\">#{match}</blockquote>"
    ...
    def process_urls
      ...
      # process website
      self.website = "http://#{website}" unless website.empty? or website.match(/^http(s)?:\/\//)
